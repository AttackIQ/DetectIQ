...
help: ## Show this help message
	...
	@echo ""
	@echo -e "\033[1;31m⚠️  IMPORTANT: Never commit or distribute .env files with API keys or secrets!\033[0m"
	@echo -e "\033[1;31m    Use .env.example as a template, but keep your .env files private.\033[0m"

# CLEAN TARGETS
.PHONY: clean/poetry-env
clean/poetry-env: ## Clean Poetry virtual environment
	@echo -e "\033[1;33m[*] Cleaning Poetry virtual environment\033[0m"
	@if poetry env info -p >/dev/null 2>&1; then \
...
clean: clean/poetry-env ## Clean backend artifacts and Poetry virtual environment
	@echo -e "\033[1;33m[*] Cleaning backend artifacts\033[0m"
	...
	@echo -e "\033[1;32m[✓] Backend cleaned\033[0m"
	@echo -e "\033[1;33m[*] Deep cleaning project (backend & poetry env)\033[0m"
	@echo -e "\033[1;32m[✓] Project cleaned\033[0m"

# INSTALL TARGETS
.PHONY: update
update: ensure-poetry-env ## Update all backend dependencies to their latest versions
	@echo -e "\033[1;33m[*] Updating '$(APP_NAME)' backend dependencies\033[0m"
	@echo "Updating backend dependencies..."
	poetry update
	@echo -e "\033[1;32m[✓] Backend dependencies updated\033[0m"

.PHONY: initialize/rulesets
initialize/rulesets: ensure-poetry-env ## Initialize all rulesets (long-running operation)
	@echo -e "\033[1;33m[*] Initializing rulesets (this may take several minutes)\033[0m"
	...
	@echo -e "\033[1;32m[✓] Rulesets initialization placeholder (command needs update)\033[0m"

.PHONY: install
install: ensure-poetry-env ## Install backend dependencies and extras
	@echo -e "\033[1;33m[*] Installing '$(APP_NAME)' backend dependencies and extras\033[0m"
	poetry install --all-extras
	@echo -e "\033[1;32m[✓] Backend dependencies installed\033[0m"
	@echo -e "\033[1;32m[!] Installing '${APP_NAME}' backend completed\033[0m"
	@echo -e "\033[1;34m[i] To initialize or update rulesets, run 'make initialize/rulesets'\033[0m"
...
build: ensure-poetry-env clean ## Build the package
	@echo -e "\033[1;33m[*] Building '$(APP_NAME)' package\033[0m"
	poetry build
...
publish: ensure-poetry-env token-check build ## Publish package to PyPI (requires prior build)
	@echo -e "\033[1;33m[*] Publishing '$(APP_NAME)' to PyPI\033[0m"
	@# Verify keyring is installed
	@if ! poetry run pip show keyring >/dev/null 2>&1; then \
		echo -e "\033[1;33m[*] Installing keyring...\033[0m"; \
